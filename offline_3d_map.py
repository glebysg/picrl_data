...
def get_3D_coordinates(filename, show_each_frame = False):
    ...
    for j in range(1080):
        colour_image_pixels_to_locate_list = [[i,j] for i in range(1920)]
    ...
    datafile_3d = open("3D."+filename+".pickle", 'wb')
    
    #make list to store the 3D positions in
    # COMENTED
    #pixel_positions_3D_list = []
    pixel_positions_3D_list = np.zeros((1080,1920,3))
    ...
    while depth_file_not_finished == True:
        try:
            ...
            for pixel in colour_image_pixels_to_locate_list:
                ...
                pixel_positions_3D_list[int(count/1920),count%(1920),:] = pixel_position_3D
                count += 1 
            ...
            # Add frame to file
            pickle.dump(pixel_positions_3D_list, datafile_3d)
            print("frames_processed: ", framenum)
            framenum +=1
...
if __name__ == '__main__': 
    ...
    three_dimensionsal_positions = get_3D_coordinates('S1.28.4.4.26', show_each_frame =  False)
    #three_dimensionsal_positions = get_3D_coordinates('S2.28.4.7.16', show_each_frame =  False)
